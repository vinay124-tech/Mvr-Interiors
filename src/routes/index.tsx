...
function HomePage() {
  ...
  return (
    <main className="overflow-hidden bg-background text-foreground">
      <header className={`fixed inset-x-0 top-0 z-50 transition-all duration-300 ${scrolled || menuOpen ? "border-b border-border bg-background/95 shadow-sm backdrop-blur" : "bg-background/80 backdrop-blur-sm"}`}>
        <div className={`mx-auto flex max-w-[1440px] items-center justify-between px-5 transition-all lg:px-10 ${scrolled ? "h-16" : "h-20"}`}>
          <a href="#home" aria-label="MVR Interiors home" className="flex items-center gap-3">
            <span className="grid h-10 w-10 place-items-center border border-primary font-display text-lg">MVR</span>
            <span><strong className="block font-display text-lg font-medium leading-none">MVR Interiors</strong><small className="mt-1 block text-[9px] uppercase tracking-[0.2em] text-muted-foreground">Design & Execution</small></span>
          </a>
      ...
      <footer className="bg-primary text-primary-foreground"><div className="mx-auto max-w-[1440px] px-5 py-12 lg:px-10"><div className="flex flex-col justify-between gap-10 border-b border-primary-foreground/20 pb-10 sm:flex-row"><div><p className="font-display text-3xl">MVR Interiors</p><p className="mt-2 text-xs uppercase tracking-[0.15em] text-primary-foreground/60">Interior Design & Execution</p></div><div className="flex items-center gap-6"><a href={contact.instagram} target="_blank" rel="noreferrer" aria-label="MVR Interiors on Instagram"><Instagram /></a><WhatsAppLink><MessageCircle /><span className="sr-only">MVR Interiors on WhatsApp</span></WhatsAppLink></div></div><div className="flex flex-col justify-between gap-3 pt-6 text-[10px] uppercase tracking-[0.12em] text-primary-foreground/55 sm:flex-row"><span>Hyderabad · Bengaluru · Nandyal · Andhra Pradesh</span><span>© {new Date().getFullYear()} MVR Interiors. All rights reserved.</span></div></div></footer>
